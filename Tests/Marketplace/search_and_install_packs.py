    ...
    try:
        ...
        if 200 <= status_code < 300:
            ...
            if dependencies_data:
                logging.info(f'Found the following dependencies for pack {pack_id}:\n{dependencies_str}')
    ...
    while not all_packs_install_successfully:
        try:
            ...
            if 200 <= status_code < 300:
                ...
                logging.debug(f'The following packs were successfully installed:\n{packs_data}')
            else:
                result_object = ast.literal_eval(response_data)
...
def upload_zipped_packs(client: demisto_client,
    ...
    try:
        ...
        if 200 <= status_code < 300:
            logging.info('All packs from {} were successfully installed!')
        else:
            result_object = ast.literal_eval(response_data)
            message = result_object.get('message', '')
            raise Exception(f'Failed to install packs - with status code {status_code}\n{message}')
