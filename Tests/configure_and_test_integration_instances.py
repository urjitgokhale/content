...
def get_content_version_details(client, ami_name):
    ...
    try:
        ...
        logging.exception(f'failed to parse response from demisto.')
        return '', 0
...
def update_content_on_demisto_instance(client, server, ami_name):
    ...
    if updating_content.lower() == 'request unsuccessful':
        ...
        if release == cd_release and asset_id == cd_asset_id:
            logging.success('Content Update Successfully Installed!')
        else:
            logging.error(
...
def instance_testing(build: Build, all_module_instances, pre_update):
    ...
    for instance in all_module_instances:
        ...
        logging.info(
            f'Testing ("Test" button) for instance "{instance_name}" of integration "{integration_of_instance}".')
...
def set_marketplace_url(servers, branch_name, ci_build_number):
    ...
    logging.info('Updated marketplace url and restarted servers, sleeping for 60 seconds')
    sleep(60)
